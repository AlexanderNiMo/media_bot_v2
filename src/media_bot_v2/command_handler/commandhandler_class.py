...
from media_bot_v2.app_enums import ClientCommands, ComponentType, ActionType, MediaType
...
class CommandMessageHandler(AppMediatorClient):
    ...
    def __init__(self, in_queue, out_queue, config: Config):
        super(self.__class__, self).__init__(in_queue, out_queue, config)
        self.db_manager = DbManager(self.config.db_cfg)
