...
from media_bot_v2.app_enums import ActionType, ClientCommands, ComponentType, MediaType
from media_bot_v2.mediator.abc_mediator_classes import MediatorMessage, MessageData
...
class MediatorActionMessage(MediatorMessage):
    ...
    def __bool__(self):
        return (
            self.__component_type is not None
            and self.__data is not None
            and self.__action is not None
        )
...
class ClientData(MessageData):
    ...
    def __repr__(self):
        return "<ClienData сообщение для пользователя {0} с текстом {1}>".format(
            self.user_id, self.message_text
        )
...
class CrawlerData(MessageData):
    def __init__(
        ...
        season=0,
        force=False,
        media_type=MediaType.BASE_MEDIA,
        *args,
        **kwargs,
    ):
    ...
    def __repr__(self):
        return "<CrawlerData from_user:{0}, kinopoisk_id:{1}, force:{2}>".format(
            self.client_id, self.media_id, self.force
        )
...
if __name__ == "__main__":
    ...
    Тест __str__
    """
    m = MediatorActionMessage(
        ComponentType.PARSER, ActionType.PARSE, ComponentType.CLIENT
    )
    print(m)
